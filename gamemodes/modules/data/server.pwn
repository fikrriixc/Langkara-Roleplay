...
CreateServerPoint()
{
	...
	if(IsValidDynamic3DTextLabel(MilkText))
            DestroyDynamic3DTextLabel(MilkText);
		
	if(IsValidDynamicPickup(MilkPickup))
		DestroyDynamicPickup(MilkPickup);
		
	//JOBS
	MatPickup = CreateDynamicPickup(1239, 23, 2484.9497, -2120.2229, 13.5468, -1, -1, -1, 5.0);
	...
	MatText = CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 2484.9497, -2120.2229, 13.5468, 5.0); // lumber
	
	CompPickup = CreateDynamicPickup(1239, 23, 315.07, 926.53, 20.46, -1, -1, -1, 5.0);
	format(strings, sizeof(strings), "[Miner]\n"WHITE_E"Component Stock: "LG_E"%d\n\n"WHITE_E"Component Price: "LG_E"%s /item\n"LB_E"'/buy'"WHITE_E" to buy\n"LB_E"'/crate sell'"WHITE_E" to restock", Component, FormatMoney(ComponentPrice));
	...
	DrugPickup = CreateDynamicPickup(1239, 23, 514.3713, -2329.4045, 508.6937, -1, -1, -1, 5.0);
	...
	DrugText = CreateDynamic3DTextLabel(strings, COLOR_GREY, 514.3713, -2329.4045, 508.6937, 5.0); // product

	MilkPickup = CreateDynamicPickup(19570, 23, 314.1242, 1147.8627, 8.5859, -1, -1, -1, 5.0);
	format(strings, sizeof(strings), "[Fresh Milk]\n"WHITE_E"Fresh Milk Stock: "LG_E"%d\n\n"WHITE_E"Fresh Milk Price: "LG_E"%s /item\n"LB_E"/sellmilk", FreshMilk, FormatMoney(FreshMilkPrice));
	MilkText = CreateDynamic3DTextLabel(strings, ARWIN, 314.1242, 1147.8627, 8.5859, 5.0); // product
}
...
Server_Save()
{
	...
	Food, FoodPrice, SeedPrice, PotatoPrice, WheatPrice, OrangePrice, Marijuana, MarijuanaPrice, FishPrice, GStationPrice, ObatMyr, ObatPrice, FreshMilk, FreshMilkPrice);
...
public:LoadServer()
{
	...
	cache_get_value_name_int(0, "freshmilk", FreshMilk);
	cache_get_value_name_int(0, "freshmilkprice", FreshMilkPrice);
