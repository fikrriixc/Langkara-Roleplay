...
new ModsPoint[MAX_MODSHOP][e_modshop_data];
...
Modshop_GetFreeID()
{
	Loop(idx, MAX_MODSHOP, 1) if(!ModsPoint[idx][ModsExist])
	{
		return idx;
	}
	return -1;
...
Player_NearModshop(playerid, Float:area = 4.0)
{
	Loop(idx, MAX_MODSHOP, 1) if(ModsPoint[idx][ModsExist] && IsPlayerInRangeOfPoint(playerid, area, ModsPoint[idx][ModsPos][0], ModsPoint[idx][ModsPos][1], ModsPoint[idx][ModsPos][2]))
	{
		return idx;
	}
	return 0;
...
{
    new String[200];
	if(PlayerData[playerid][pAdmin] < 5) return PermissionError(playerid);
 	
 	new idx;
 	if((idx = Modshop_GetFreeID()) == -1)
 		return SendErrorMessage(playerid, "Modshop sudah full!");
...
CMD:editmpoint(playerid, params[])
{
	...
	{
	    SendSyntaxMessage(playerid, "/editmpoint [Opsi]");
	    SendInfoMessage(playerid, "Posisi");
	    return 1;
	}
	if(!strcmp(params, "posisi", true, 6))
	{
	    if(sscanf(params, "s[32]i", params, id)) return SendSyntaxMessage(playerid, "/editmpoint posisi [id]");
		if(!ModsPoint[id][ModsExist])
		{
			SendErrorMessage(playerid, "invalid ModsPoint id");
			return 1;
		}
		new Float:X, Float:Y, Float:Z;
  		GetPlayerPos(playerid, X, Y, Z);
...
CMD:deletempoint(playerid, params[])
{
	new idx;
	if(PlayerData[playerid][pAdmin] < 5) return PermissionError(playerid);
	if(sscanf(params, "i", idx)) return SendSyntaxMessage(playerid, "/deletempoint [id]");
	if(!ModsPoint[idx][ModsExist])
	{
		SendErrorMessage(playerid, "invalid ModsPoint id");
		return 1;
	}
