...
CMD:gotodoor(playerid, params[])
{
	...
	SendServerMessage(playerid, "You have teleport to door id %d", did);
	return 1;
}
CMD:editdoor(playerid, params[])
{
    ...
    {
        SendSyntaxMessage(playerid, "/editdoor [id] [name]");
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "%s has adjusted the location of entrance ID: %d.", PlayerData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "interior", true))
    {
       ...
       /*foreach (new i : Player)
        ...
        SendAdminMessage(COLOR_RED, "%s has adjusted the interior spawn of entrance ID: %d.", PlayerData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "custom", true))
    {
        ...
        if(sscanf(string, "d", status))
            return SendSyntaxMessage(playerid, "/editdoor [id] [custom] [0/1]");
        ...
        if(status) {
            SendAdminMessage(COLOR_RED, "%s has enabled custom interior mode for entrance ID: %d.", PlayerData[playerid][pAdminname], did);
        }
        else {
            SendAdminMessage(COLOR_RED, "%s has disabled custom interior mode for entrance ID: %d.", PlayerData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "virtual", true))
    {
        ...
        if(sscanf(string, "d", worldid))
            return SendSyntaxMessage(playerid, "/editdoor [id] [virtual] [interior world]");

        dData[did][dExtvw] = worldid;

        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_RED, "%s has adjusted the virtual of entrance ID: %d to %d.", PlayerData[playerid][pAdminname], did, worldid);
    }
    else if(!strcmp(type, "password", true))
    {
        ...
        if(sscanf(string, "s[32]", password))
            return SendSyntaxMessage(playerid, "/editdoor [id] [password] [entrance pass] (use 'none' to disable)");
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_RED, "%s has adjusted the password of entrance ID: %d to %s", PlayerData[playerid][pAdminname], did, password);
    }
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(sscanf(string, "d", locked))
            return SendSyntaxMessage(playerid, "/editdoor [id] [locked] [locked 0/1]");
        ...
        if(locked) {
            SendAdminMessage(COLOR_RED, "%s has locked entrance ID: %d.", PlayerData[playerid][pAdminname], did);
        } else {
            SendAdminMessage(COLOR_RED, "%s has unlocked entrance ID: %d.", PlayerData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "name", true))
    {
        ...
        if(sscanf(string, "s[128]", name))
            return SendSyntaxMessage(playerid, "/editdoor [id] [name] [new name]");
        ...
        SendAdminMessage(COLOR_RED, "%s has adjusted the name of entrance ID: %d to \"%s\".", PlayerData[playerid][pAdminname], did, ColouredText(name));
    }
	else if(!strcmp(type, "admin", true))
    {
        ...
        if(sscanf(string, "d", level))
            return SendSyntaxMessage(playerid, "/editdoor [id] [admin] [level]");
        ...
        SendAdminMessage(COLOR_RED, "%s has set entrance ID: %d to admin level %d.", PlayerData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "vip", true))
    {
        ...
        if(sscanf(string, "d", level))
            return SendSyntaxMessage(playerid, "/editdoor [id] [VIP] [level]");
        ...
        SendAdminMessage(COLOR_RED, "%s has set entrance ID: %d to VIP level %d.", PlayerData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "faction", true))
    {
        ...
        if(sscanf(string, "d", fid))
            return SendSyntaxMessage(playerid, "/editdoor [id] [faction] [faction id]");

        if(fid < 0 || fid > 5)
            return SendErrorMessage(playerid, "Invalid value. Use 0 - 5 for type.");

        dData[did][dFaction] = fid;
        Doors_Save(did);
		Doors_Updatelabel(did);

        SendAdminMessage(COLOR_RED, "%s has set entrance ID: %d to faction id %d.", PlayerData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "family", true))
    {
        ...
        if(sscanf(string, "d", fid))
            return SendSyntaxMessage(playerid, "/editdoor [id] [family] [family id]");
        ...
        SendAdminMessage(COLOR_RED, "%s has set entrance ID: %d to family id %d.", PlayerData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "garage", true))
	{
        ...
        if(sscanf(string, "d", gid))
            return SendSyntaxMessage(playerid, "/editdoor [id] [garage] [0 - 1]");
		...
		{
			dData[did][dGarage] = 0;
			SendAdminMessage(COLOR_RED, "%s has set entrance ID: %d to garage vehicle disable.", PlayerData[playerid][pAdminname], did);
		}
		else
		{
			dData[did][dGarage] = 1;
			SendAdminMessage(COLOR_RED, "%s has set entrance ID: %d to garage vehicle enable.", PlayerData[playerid][pAdminname], did);
		}
		Doors_Save(gid);
	...
	else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "%s has delete door ID: %d.", PlayerData[playerid][pAdminname], did);
	}
    return 1;
}