...
/*MySQL_LoadPlayerToys(playerid)
{
	...
	mysql_format(g_SQL, tstr, sizeof(tstr), " FROM toys WHERE Owner='%s' LIMIT 1", PlayerData[playerid][pName]);
...
public:LoadPlayerToys(playerid)
{
	...
	if(rows)
	{
		PlayerData[playerid][PurchasedToy] = true;
		...
		printf("[P_TOYS] Success loaded from %s(%d)", PlayerData[playerid][pName], playerid);
	}
	return true;
}*/
...
{
	new tstr[512];
	mysql_format(g_SQL, tstr, sizeof(tstr), "SELECT * FROM toys WHERE Owner='%s' LIMIT 1", PlayerData[playerid][pName]);
...
{
	new query[512];
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `toys` (`Owner`) VALUES ('%s');", PlayerData[playerid][pName]);
...
{
	if(PlayerData[playerid][PurchasedToy] == false) return 1;
...
{
	if(PlayerData[playerid][PurchasedToy] == false) return true;
...
CMD:toys(playerid)
{
	if(IsPlayerInAnyVehicle(playerid)) return SendErrorMessage(playerid, "This command can only be used on foot, exit your vehicle!");
	if(PlayerData[playerid][IsLoggedIn] == false) return SendErrorMessage(playerid, "You must be logged in to attach objects to your character!");
	new string[350];
	if(pToys[playerid][0][toy_model] == 0)
	{
