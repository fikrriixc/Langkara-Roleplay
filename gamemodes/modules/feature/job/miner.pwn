...
{
	if(!IsPlayerConnected(playerid) || PlayerData[playerid][MiningOreID] == -1) return 0;
	new id = PlayerData[playerid][MiningOreID];
...
{
	if(!IsPlayerConnected(playerid) || PlayerData[playerid][CarryingLog] == -1) return 0;
	RemovePlayerAttachedObject(playerid, 9);
	SetPlayerSpecialAction(playerid, SPECIAL_ACTION_NONE);
	PlayerData[playerid][CarryingLog] = -1;
	
	if(PlayerData[playerid][LoadingPoint] != -1)
	{
	    DestroyDynamicCP(PlayerData[playerid][LoadingPoint]);
	    PlayerData[playerid][LoadingPoint] = -1;
	}
	return 1;
}

public:MiningOre(playerid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	if(!IsValidTimer(PlayerData[playerid][pActivity])) return 0;
    if(PlayerData[playerid][MiningOreID] != -1)
	{
		new tid = PlayerData[playerid][MiningOreID];
		
		if(PlayerData[playerid][pActivityTime] >= 100 && PlayerData[playerid][MiningOreID] != -1)
		{
...
{
    if(!IsPlayerConnected(playerid)) return 0;
	PlayerData[playerid][CarryingLog] = type;
    ...
    //PlayerData[playerid][LoadingPoint] = CreateDynamicCP(x, y, z, 3.0, -1, -1, playerid, 10.0);
...
{
	if(!Iter_Contains(Ores, id)) return 0;
	foreach(new i : Player) if(PlayerData[i][pEditType] == EDIT_ORE && PlayerData[i][pEditID] == id) return 1;
	return 0;
}
...
CMD:createore(playerid, params[])
{
	...
	if(tid == -1) return SendErrorMessage(playerid, "Can't add any more ores.");
	
	if(sscanf(params, "d", type)) return SendSyntaxMessage(playerid, "/createore [type, 0.Metal 1.Coal");
	if(type > 1 || type < 0) return SendErrorMessage(playerid, "ore type only id 0 and 1.");
...
public:OnOreCreated(playerid, tid)
{
	Ore_Save(tid);
	
	PlayerData[playerid][pEditType] = EDIT_ORE;
	PlayerData[playerid][pEditID] = tid;
	EditDynamicObject(playerid, OreData[tid][oreObjID]);
	SendServerMessage(playerid, "Ore created.");
...
CMD:editore(playerid, params[])
{
	...
	new tid;
	if(sscanf(params, "i", tid)) return SendSyntaxMessage(playerid, "/editore [id]");
	if(!Iter_Contains(Ores, tid)) return SendErrorMessage(playerid, "Invalid ID.");
	if(OreData[tid][oreGettingMiner]) return SendErrorMessage(playerid, "Can't edit specified ore because its getting miner.");
	if(!IsPlayerInRangeOfPoint(playerid, 30.0, OreData[tid][oreX], OreData[tid][oreY], OreData[tid][oreZ])) return SendErrorMessage(playerid, "You're not near the ore you want to edit.");
	PlayerData[playerid][pEditType] = EDIT_ORE;
	PlayerData[playerid][pEditID] = tid;
	EditDynamicObject(playerid, OreData[tid][oreObjID]);
	return 1;
...
CMD:removeore(playerid, params[])
{
	...
	new tid, query[512];
	if(sscanf(params, "i", tid)) return SendSyntaxMessage(playerid, "/removeore [id]");
	if(!Iter_Contains(Ores, tid)) return SendErrorMessage(playerid, "Invalid ID.");
	if(OreData[tid][oreGettingMiner]) return SendErrorMessage(playerid, "Can't remove specified ore because its getting cut down.");
	if(Ore_BeingEdited(tid)) return SendErrorMessage(playerid, "Can't remove specified ore because its being edited.");
	...
	SendServerMessage(playerid, "Ore removed.");
	return 1;
}
...
CMD:gotoore(playerid, params[])
{
	...
	if(sscanf(params, "d", tid))
		return SendSyntaxMessage(playerid, "/gotoore [id]");
	if(!Iter_Contains(Ores, tid)) return SendErrorMessage(playerid, "The ore you specified ID of doesn't exist.");
...
CMD:ore(playerid, params[])
{
	...
	{
		if(isnull(params)) return SendSyntaxMessage(playerid, "/ore [info/mine/pickup/sell]");
		...
		if(!strcmp(params, "mine", true))
		{
			...
			new logtype = PlayerData[playerid][CarryingLog];
			if(PlayerData[playerid][CarryingLog] != -1) return Player_DropLog(playerid, logtype);
			
			if(GetPlayerWeapon(playerid) != WEAPON_SHOVEL) return SendErrorMessage(playerid, "Anda butuh skop/shovel.");
				
			if(!IsATruck(GetPVarInt(playerid, "LastVehicleID"))) return SendErrorMessage(playerid, "Your last vehicle isn't available for mining.");
			...
			if(tid != -1)
			{
				...
				{
					if(OreData[tid][oreLog] < 1) return SendErrorMessage(playerid, "This ore log is empty.");
					new Float: x, Float: y, Float: z;
					GetVehicleBoot(GetPVarInt(playerid, "LastVehicleID"), x, y, z);
					if(GetPlayerDistanceFromPoint(playerid, x, y, z) > 60.0) return SendErrorMessage(playerid, "Your last vehicle is too far away.");
					...
					PlayerData[playerid][pActivity] = SetTimerEx("MiningOre", 1000, true, "i", playerid);
					PlayerData[playerid][MiningOreID] = tid;
				...
				else return SendErrorMessage(playerid, "This ore is not ready.");
			}
			else return SendErrorMessage(playerid, "You must near on the ore area");
		}
		else if(!strcmp(params, "pickup", true))
		{
			if(IsPlayerInAnyVehicle(playerid)) return SendErrorMessage(playerid, "Anda harus keluar dari kendaraan.");
			// taking from ground
			if(PlayerData[playerid][CarryingLog] != -1) return SendErrorMessage(playerid, "You're already carrying a log.");
		...
		else if(!strcmp(params, "info", true))
		{
			...
			if(!IsValidVehicle(vehicleid)) return SendErrorMessage(playerid, "You're not near any vehicle.");
			if(!IsATruck(vehicleid)) return SendErrorMessage(playerid, "Vehicle isn't a mining vehicle.");
		...
		{
			if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendErrorMessage(playerid, "You are not driver in ore truck.");
			new vehicleid = GetPlayerVehicleID(playerid);
			if(!IsValidVehicle(vehicleid)) return SendErrorMessage(playerid, "You're not in any vehicle.");
			if(!IsATruck(vehicleid)) return SendErrorMessage(playerid, "Vehicle isn't a mining vehicle.");
			if(!IsPlayerInRangeOfPoint(playerid, 4.0, 293.73, 913.17, 20.40)) return SendErrorMessage(playerid, "You're not near a miner warehouse.");
			new cash = Vehicle_GetOreValue(vehicleid), carid;
			if(cash < 1) return SendErrorMessage(playerid, "This vehicle ore is empty!");
			SendInfoMessage(playerid, "Sold "BLUE_E"%d "WHITE_E"ores and earned "LG_E"%s.", Vehicle_LogCount(vehicleid), FormatMoney(cash));
			...
			{
				VehicleData[carid][cMetal] = 0;
				VehicleData[carid][cCoal] = 0;
			}
			PlayerData[playerid][pJobTime] += 150;

			#pragma unused carid
		}
	}
	else return SendErrorMessage(playerid, "You are not miner job.");
	return 1;
}
