...
{
    if(!strcmp(wsEmploy[id][0], PlayerData[playerid][pName], true)) return 1;
    if(!strcmp(wsEmploy[id][1], PlayerData[playerid][pName], true)) return 1;
    if(!strcmp(wsEmploy[id][2], PlayerData[playerid][pName], true)) return 1;
    return 0;
}

IsWorkshopOwner(playerid, id)
{
    return (wsData[id][wOwnerID] == PlayerData[playerid][pID]) || (!strcmp(wsData[id][wOwner], PlayerData[playerid][pName], true));
}
...
GetOwnedWorkshop(playerid)
{
	...
	{
	    if(!strcmp(wsData[wid][wOwner], PlayerData[playerid][pName], true) || (wsData[wid][wOwnerID] == PlayerData[playerid][pID]))
	    {
     		tmpcount++;
		}
	}
...
ReturnPlayerWorkshopID(playerid, hslot)
{
	...
	foreach(new wid : Workshop)
	{
	    if(!strcmp(PlayerData[playerid][pName], wsData[wid][wOwner], true))
	    {
     		tmpcount++;
       		if(tmpcount == hslot)
       		{
    ...
    foreach(new id : Workshop)
	{
        if(IsPlayerInRangeOfPoint(playerid, 4.0, wsData[id][wX], wsData[id][wY], wsData[id][wZ]))
        {
            if(!IsWorkshopOwner(playerid, id) && !IsWorkshopEmploye(playerid, id))
                return SendErrorMessage(playerid, "You're not the Owner or Employee of this Workshop");
...
IsPlayerNearWorkshop(playerid, Float:range = 30.0)
{
	foreach(new fan : Workshop) if(IsPlayerInRangeOfPoint(playerid, range, wsData[fan][wX], wsData[fan][wY], wsData[fan][wZ]))
		return fan;
...
CMD:createworkshop(playerid, params[])
{
	...
	if(wid == -1) return SendErrorMessage(playerid, "You cant create more workshop!");
	new price;
	if(sscanf(params, "d", price)) return SendSyntaxMessage(playerid, "/createworkshop [price]");
	new totalcash[25];
    ...
    SendInfoMessage(playerid, "Created Workshop ID:%d", wid);
	return 1;
}

public:OnWorkshopCreated(wid)
{
	Workshop_Save(wid);
    Workshop_Refresh(wid);
	return 1;
...
CMD:gotoworkshop(playerid, params[])
{
	...
	if(sscanf(params, "d", wid))
		return SendSyntaxMessage(playerid, "/gotoworkshop [id]");
	if(!Iter_Contains(Workshop, wid)) return SendErrorMessage(playerid, "The Workshop you specified ID of doesn't exist.");
	SetPlayerPos(playerid, wsData[wid][wX], wsData[wid][wY], wsData[wid][wZ]);
    SetPlayerInterior(playerid, 0);
    SetPlayerVirtualWorld(playerid, 0);
	SendInfoMessage(playerid, "You has teleport to Workshop id %d", wid);
	return 1;
}
...
CMD:editworkshop(playerid, params[])
{
    ...
    {
        SendSyntaxMessage(playerid, "/editworkshop [id] [name]");
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "%s has adjusted the location of Workshop ID: %d.", PlayerData[playerid][pAdminname], wid);
    }
    else if(!strcmp(type, "status", true))
    {
        ...
        if(sscanf(string, "d", locked))
            return SendSyntaxMessage(playerid, "/editWorkshop [id] [locked] [0/1]");
        ...
        if(locked) {
            SendAdminMessage(COLOR_RED, "%s has Opened Workshop ID: %d.", PlayerData[playerid][pAdminname], wid);
        }
        else {
            SendAdminMessage(COLOR_RED, "%s has Closed Workshop ID: %d.", PlayerData[playerid][pAdminname], wid);
        }
    }
    else if(!strcmp(type, "price", true))
    {
        ...
        if(sscanf(string, "d", price))
            return SendSyntaxMessage(playerid, "/editWorkshop [id] [Price] [Amount]");

        wsData[wid][wPrice] = price;

        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the price of Workshop ID: %d to %d.", PlayerData[playerid][pAdminname], wid, price);
    }
	else if(!strcmp(type, "money", true))
    {
        ...
        if(sscanf(string, "d", money))
            return SendSyntaxMessage(playerid, "/editWorkshop [id] [money] [Ammount]");

        wsData[wid][wMoney] = money;
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the money of Workshop ID: %d to %s.", PlayerData[playerid][pAdminname], wid, FormatMoney(money));
    }
	else if(!strcmp(type, "comp", true))
    {
        ...
        if(sscanf(string, "d", amount))
            return SendSyntaxMessage(playerid, "/editWorkshop [id] [Comp] [Ammount]");

        wsData[wid][wComp] = amount;
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the component of Workshop ID: %d to %d.", PlayerData[playerid][pAdminname], wid, amount);
    }
    else if(!strcmp(type, "mat", true))
    {
        ...
        if(sscanf(string, "d", amount))
            return SendSyntaxMessage(playerid, "/editWorkshop [id] [mat] [Ammount]");

        wsData[wid][wMat] = amount;
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the material of Workshop ID: %d to %d.", PlayerData[playerid][pAdminname], wid, amount);
    }
    else if(!strcmp(type, "owner", true))
    {
        ...
        if(sscanf(string, "s[24]", owners))
            return SendSyntaxMessage(playerid, "/editWorkshop [id] [owner] [player name] (use '-' to no owner)");

        format(wsData[wid][wOwner], MAX_PLAYER_NAME, owners);
  
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the owner of Workshop ID: %d to %s", PlayerData[playerid][pAdminname], wid, owners);
    }
    else if(!strcmp(type, "reset", true))
    {
		...
		Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has reset Workshop ID: %d.", PlayerData[playerid][pAdminname], wid);
    }
	else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "%s has delete Workshop ID: %d.", PlayerData[playerid][pAdminname], wid);
	}
    return 1;
}
...
CMD:lockws(playerid, params[])
{
	foreach(new wid : Workshop)
	{
		...
		{
			if(!IsWorkshopOwner(playerid, wid) && !IsWorkshopEmploye(playerid, wid)) return SendErrorMessage(playerid, "Kamu bukan pengurus Workshop ini.");
...
CMD:myws(playerid)
{
	if(!GetOwnedWorkshop(playerid)) return SendErrorMessage(playerid, "You don't have any Workshop.");
